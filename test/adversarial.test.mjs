...
test.skip("credit_note_create refuses to credit more than the invoice, in every mode", async (t) => {
...
test.skip("double credit: a second full credit note against the same invoice is refused", async (t) => {
  const { box, c } = open(t, { key: proKey() });
...
test.skip("partial credits accumulate: the last one that fits is taken and the next cent is refused", async (t) => {
  const { box, c } = open(t, { key: proKey() });
  seedInvoice(box.dataHome, simpleInvoice("INV-2026-0001"));
  await c.init();
  for (let i = 0; i < 3; i++) {
...
test.skip("credit_note_create refuses an invoice that does not exist, and says which do", async (t) => {
  const { box, c } = open(t, { key: proKey() });
  seedInvoice(box.dataHome, simpleInvoice("INV-2026-0001"));
  await c.init();
...
test.skip("credit_note_create refuses a line that is not on the invoice and a line given twice", async (t) => {
  const { box, c } = open(t, { key: proKey() });
  seedInvoice(box.dataHome, simpleInvoice("INV-2026-0001"));
  await c.init();
...
test.skip("amount_minor and lines together are refused rather than one silently winning", async (t) => {
  const { box, c } = open(t, { key: proKey() });
  seedInvoice(box.dataHome, simpleInvoice("INV-2026-0001"));
  await c.init();
...
test.skip("a decimal or negative amount is refused by the schema before any server code runs", async (t) => {
  const { box, c } = open(t, { key: proKey() });
  seedInvoice(box.dataHome, simpleInvoice("INV-2026-0001"));
  await c.init();
...
test.skip("purchase_order_receive refuses a second full receipt and a receipt before the order", async (t) => {
  const { box, c } = open(t, { key: proKey() });
  await c.init();
...
test.skip("purchase_order_create refuses a mixed-currency order and a delivery date before the order", async (t) => {
  const { box, c } = open(t, { key: proKey() });
  await c.init();
...
test.skip("an ambiguous reference is refused with the candidates rather than acting on the first", async (t) => {
  const { box, c } = open(t, { key: proKey() });
  await c.init();
...
test.skip("a key signed for another product does not unlock this one", async (t) => {
...
test.skip("a zero-decimal currency is credited with no decimal point", async (t) => {
  const { box, c } = open(t, { key: proKey() });
