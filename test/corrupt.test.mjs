...
test.skip("a corrupt credit note store is quarantined byte-for-byte and every later call fails", async (t) => {
  const box = sandbox();
...
test.skip("a corrupt purchase order store blocks the orders and leaves the credit notes readable", async (t) => {
  const box = sandbox();
  const dir = docsDir(box.dataHome);
  mkdirSync(dir, { recursive: true });
