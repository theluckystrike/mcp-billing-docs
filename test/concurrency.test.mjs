...
test.skip("40 concurrent purchase orders from two processes get 40 unique ids", async (t) => {
  const box = sandbox();
  const key = proKey();
...
test.skip("concurrent credit notes cannot together exceed the invoice", async (t) => {
  const box = sandbox();
  const key = proKey();
