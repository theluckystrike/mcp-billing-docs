...
test.skip("a paid invoice can still be credited: payment and credit are separate facts", async (t) => {
...
test.skip("credit_note_create has no currency parameter: a credit note can never disagree with its invoice's currency", async (t) => {
  const { box, c } = open(t, { key: proKey() });
...
test.skip("amount_minor: 0 is refused at the schema, before any server code runs", async (t) => {
  const { box, c } = open(t, { key: proKey() });
